...
$('.search_in_table').keyup(function () {
    $(this).parent().closest('.card').next().find('table').DataTable().search($(this).val()).draw();
});

$('.length_change ul li a').on('click', function () {
    $(this).parent().closest('.card').next().find('table').DataTable().page.len($(this).data('value')).draw();
});