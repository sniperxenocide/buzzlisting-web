 ...
 */
{
    let wf_activity_status,
        ...
        default_url = '/api/v1/workflow/delegation_report/';
    getWfActivity(default_url);
    $selectStatus.on('change', function () {
            let url = '/api/v1/workflow/delegation_report/?operation=activity&status=' + this.value;
            if (this.value == "all") {
                url = '/api/v1/workflow/delegation_report/';
            } else {
                url = '/api/v1/workflow/delegation_report/?operation=activity&status=' + this.value;
            }
            // console.log(url);
            // if(this.value = "all"){
            //     url = '/api/v1/workflow/delegation_report/';
            // }
            wf_activity_status.destroy();
            getWfActivity(url);
        });
    function getWfActivity(url) {
        console.log(url);
        wf_activity_status = $('#wf_activity_status').DataTable({
            processing: true,
            serverSide: true,
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    width: "8%",
                    render: (data, a, b) => {
                        let html = '<a href="#" id="wf_app_number" data-id=' + b.app_id + '>' + b.app_number + '</a>';
                        return html;
                    }
                },
                {
        ...
        $dateRangeField.on('apply.daterangepicker', function (ev, picker) {
            ...
            wf_activity_status.iCacheLower = -1;
            wf_activity_status.clearPipeline();
            wf_activity_status.columns(1).search(from);
            wf_activity_status.columns(2).search(to);
            wf_activity_status.draw(false);
        });
